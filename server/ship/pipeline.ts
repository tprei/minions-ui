...
class SessionHandle implements SessionPort {
  ...
  waitForCompletion(): Promise<SessionDoneState> { return Promise.resolve("completed") }
  isClosed(): boolean { return false }
  getState(): SessionState { return "idle" }
...
function formatShipPhaseAdvance(slug: string, from: string, to: string): string {
  return `🚢 <b>Ship: ${esc(from)} complete</b>  ·  🏷 <code>${esc(slug)}</code>\n\nAdvancing to <b>${esc(to)}</b> phase…`
}

function formatShipComplete(slug: string, passed: number, failed: number, total: number): string {
