...
export function NewTaskBar({ store, navigate = defaultNavigate }: NewTaskBarProps) {
  ...
  if (repos.length > 0 && !userPickedRepo.value && !repo.value) {
    repo.value = repos[0].alias
  }

  const pickRepo = (alias: string) => {
    repo.value = alias
    userPickedRepo.value = true
  }
    ...
    >
      <div class="flex flex-wrap items-center gap-2">
        ...
        {repos.length > 0 && (
          ...
          >
            {repos.map((r) => {
              const active = repo.value === r.alias
              ...
              return (
                <button
                  ...
                  disabled={sending.value}
                  title={r.url}
                  onClick={() => { pickRepo(r.alias) }}
                  data-testid={`new-task-repo-chip-${r.alias}`}
                  class={`rounded-full border px-2.5 py-1 text-xs font-medium transition-colors disabled:opacity-50 ${chipClass}`}
                >
                  {r.alias}
                </button>
              )
            ...
            <button
              type="button"
              role="radio"
              aria-checked={repo.value === ''}
              disabled={sending.value}
              title="Run without a specific repo"
              onClick={() => { pickRepo('') }}
              ...
              class={`rounded-full border px-2.5 py-1 text-xs font-medium transition-colors disabled:opacity-50 ${
                repo.value === ''
                  ? 'bg-indigo-600 text-white border-indigo-700'
                  : 'bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700'
              }`}
            >
              (no repo)
            </button>
