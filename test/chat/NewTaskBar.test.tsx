...
describe('NewTaskBar', () => {
  ...
  describe('repo chip strip', () => {
    it('renders a chip per repo plus a "(no repo)" chip, first repo selected by default', () => {
      const { store } = makeStore({
        features: ['sessions-create'],
        repos: [
          { alias: 'api', url: 'https://github.com/org/api' },
          { alias: 'web', url: 'https://github.com/org/web' },
        ],
      })
      render(<NewTaskBar store={store} />)
      const chips = screen.getByTestId('new-task-repo-chips')
      expect(chips.getAttribute('role')).toBe('radiogroup')
      expect(screen.getByTestId('new-task-repo-chip-api')).toBeTruthy()
      expect(screen.getByTestId('new-task-repo-chip-web')).toBeTruthy()
      expect(screen.getByTestId('new-task-repo-chip-none')).toBeTruthy()
      expect(screen.getByTestId('new-task-repo-chip-api').getAttribute('aria-checked')).toBe('true')
      expect(screen.getByTestId('new-task-repo-chip-web').getAttribute('aria-checked')).toBe('false')
      expect(screen.getByTestId('new-task-repo-chip-none').getAttribute('aria-checked')).toBe('false')
    })

    it('clicking a repo chip updates the selected repo', () => {
      const { store } = makeStore({
        features: ['sessions-create'],
        repos: [
          { alias: 'api', url: 'https://github.com/org/api' },
      ...
      render(<NewTaskBar store={store} />)
      fireEvent.click(screen.getByTestId('new-task-repo-chip-web'))
      expect(screen.getByTestId('new-task-repo-chip-web').getAttribute('aria-checked')).toBe('true')
      expect(screen.getByTestId('new-task-repo-chip-api').getAttribute('aria-checked')).toBe('false')
    })

    it('does not render chip strip when no repos are advertised', () => {
      const { store } = makeStore({ features: ['sessions-create'], repos: [] })
      render(<NewTaskBar store={store} />)
      expect(screen.queryByTestId('new-task-repo-chips')).toBeNull()
    })

    it('"(no repo)" chip omits repo from createSession payload', async () => {
      const { store, client } = makeStore({
        features: ['sessions-create'],
        repos: [{ alias: 'api', url: 'https://github.com/org/api' }],
      })
      render(<NewTaskBar store={store} />)
      fireEvent.click(screen.getByTestId('new-task-repo-chip-none'))
      const textarea = screen.getByTestId('new-task-prompt') as HTMLTextAreaElement
    ...
    it('selected repo chip is sent as repo field on createSession', async () => {
      const { store, client } = makeStore({
        features: ['sessions-create'],
        repos: [
          { alias: 'api', url: 'https://github.com/org/api' },
